...
extension SwiftBetterPlayerPlugin {
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        if call.method == "setHlsToken" {
            let args = call.arguments as? [String: Any]
            let token = args?["token"] as? String
            let exp = args?["exp"] as? String
            BetterPlayerHlsTokenStore.update(token: token, exp: exp)
            result(nil)
            return
        }
    ...
    private func refreshHlsTokenForPlayers() {
        for (textureId, player) in players {
            guard let dataSource = dataSourceDict[textureId] else { continue }
            let uriArg = dataSource["uri"] as? String
            guard let uriArg = uriArg, let url = URL(string: uriArg) else { continue }
            if url.pathExtension.lowercased() != "m3u8" { continue }
