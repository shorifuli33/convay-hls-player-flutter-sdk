...
public class BetterPlayer: NSObject, FlutterPlatformView, FlutterStreamHandler, AVPictureInPictureControllerDelegate {
    ...
    private var hlsTokenLoaderDelegate: BetterPlayerHlsTokenResourceLoaderDelegate?
    ...
    @objc private func itemFailedToPlayToEndTime(_ notification: Notification) {
        ...
        if let error = error {
            NSLog("BetterPlayer item failed to play to end: domain=%@ code=%ld desc=%@", error.domain, error.code, error.localizedDescription)
        } else {
            NSLog("BetterPlayer item failed to play to end: unknown error")
        }
        logPlayerItemErrors(item)
    }

    private func logPlayerItemErrors(_ item: AVPlayerItem) {
        if let errorLog = item.errorLog() {
            NSLog("BetterPlayer error log entries: %d", errorLog.events.count)
            for event in errorLog.events {
                NSLog("BetterPlayer error log: status=%ld uri=%@ domain=%@ comment=%@", event.errorStatusCode, event.uri ?? "-", event.errorDomain ?? "-", event.errorComment ?? "-")
            }
        }
        if let accessLog = item.accessLog() {
            NSLog("BetterPlayer access log entries: %d", accessLog.events.count)
            for event in accessLog.events {
                NSLog("BetterPlayer access log: uri=%@ bytes=%ld duration=%f", event.uri ?? "-", event.numberOfBytesTransferred, event.durationWatched)
            }
        }
    }
    ...
    public func setDataSourceURL(_ url: URL, key: String?, certificateUrl: String?, licenseUrl: String?, headers: [AnyHashable: Any], useCache: Bool, cacheKey: String?, cacheManager: CacheManager, overriddenDuration: Int, videoExtension: String?) {
        ...
        let tokenHeader = finalHeaders["X-HLS-Token"] as? String
        let expHeader = finalHeaders["X-HLS-Exp"] as? String
        if tokenHeader != nil || expHeader != nil {
            BetterPlayerHlsTokenStore.update(token: tokenHeader, exp: expHeader)
        }
        ...
        if useCache {
            ...
            var assetUrl = url
            let scheme = assetUrl.scheme ?? "https"
            let isHttp = scheme == "http" || scheme == "https"
            let isHls = assetUrl.pathExtension.lowercased() == "m3u8"
            if isHttp && isHls && BetterPlayerHlsTokenStore.hasToken() {
                if var components = URLComponents(url: assetUrl, resolvingAgainstBaseURL: false) {
                    components.scheme = BetterPlayerHlsTokenResourceLoaderDelegate.tokenScheme
                    if let updatedUrl = components.url {
                        assetUrl = updatedUrl
                        hlsTokenLoaderDelegate = BetterPlayerHlsTokenResourceLoaderDelegate(
            ...
            let asset = AVURLAsset(url: assetUrl, options: ["AVURLAssetHTTPHeaderFieldsKey": finalHeaders])
            if let tokenDelegate = hlsTokenLoaderDelegate {
                asset.resourceLoader.setDelegate(tokenDelegate, queue: DispatchQueue.main)
            }
    ...
    private func softRefreshCurrentItem() -> Bool {
    ...
    public func refreshHlsToken(url: URL, key: String?, certificateUrl: String?, licenseUrl: String?, headers: [AnyHashable: Any], useCache: Bool, cacheKey: String?, cacheManager: CacheManager, overriddenDuration: Int, videoExtension: String?) {
        if softRefreshCurrentItem() {
            return
        }
        let wasPlaying = isPlaying
        let resumePosition = position()
        setDataSourceURL(url, key: key, certificateUrl: certificateUrl, licenseUrl: licenseUrl, headers: headers, useCache: useCache, cacheKey: cacheKey, cacheManager: cacheManager, overriddenDuration: overriddenDuration, videoExtension: videoExtension)
        if resumePosition > 0 { seekTo(Int(resumePosition)) }
        if wasPlaying { play() }
    }
            ...
            if let item = object as? AVPlayerItem {
                ...
                case .failed:
                    if let error = item.error as NSError? {
                        NSLog("Failed to load video: domain=%@ code=%ld desc=%@", error.domain, error.code, error.localizedDescription)
                    } else {
                        NSLog("Failed to load video: unknown error")
                    }
                    logPlayerItemErrors(item)
                    if let eventSink = eventSink {
